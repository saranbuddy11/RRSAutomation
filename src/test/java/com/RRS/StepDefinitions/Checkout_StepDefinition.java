...
public class Checkout_StepDefinition extends baseClass{
	...
	public void assert_order_summary_is_updated_with_the_new_shipping_fees() {
		...
		try {
			...
			log.info("Shipping Fee After change in float: "+Shippping_After);
			float Shipping_Before=Float.parseFloat(propertyFileModifier.getProperty("ShippingFee"));
	...
	public void user_use_credit_card_payment_method_payment_method_in_the_checkout_page(String PaymentMethod) {
		...
		try {
			CheckoutS3Page.payment_ccNewCard("Mohanraj", PaymentDetails[0], PaymentDetails[1], PaymentDetails[2], PaymentDetails[3]);
